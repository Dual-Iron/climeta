use super::{bits, Field, MethodDefSig, ParamKind, PrimitiveType, Type, TypeTag};
...
fn read_string<'db>(cursor: &mut &'db [u8]) -> Result<Option<&'db str>> {
    ...
    Ok(Some(std::str::from_utf8(left).map_err(|_| {
        crate::DecodeError("Invalid UTF8 in constant value")
    })?))
}
...
impl<'db> CustomAttributeSig<'db> {
    pub(crate) fn parse<'c: 'db>(
        cur: &mut &'db [u8],
        db: &'db Database<'db>,
        cache: &Cache<'c>,
        ctor: &MethodDefSig<'db>,
    ) -> Result<CustomAttributeSig<'db>> {
        ...
        for param in ctor_params {
            let elem_kind = match param.kind() {
                ParamKind::Type(t) => ElemKind::from_fixed_arg_type(t, cache)?,
...
impl<'db> NamedArg<'db> {
    fn parse<'c: 'db>(
        ...
        if is_property {
            Ok(NamedArg {
                name: NamedArgName::Property(name),
                value,
            })
        } else {
            Ok(NamedArg {
                name: NamedArgName::Field(name),
                value,
            })
        }
    }
...
impl<'db> FieldOrPropType<'db> {
    fn from_fixed_arg_type<'c: 'db>(
        typ: &Type<'db>,
        cache: &Cache<'c>,
    ) -> Result<FieldOrPropType<'db>> {
        Ok(match typ {
            Type::Primitive(PrimitiveType::I) | Type::Primitive(PrimitiveType::U) => {
                return Err("FieldOrPropType can not have type I or U".into())
            }
            Type::Primitive(p) => FieldOrPropType::Primitive(*p),
            Type::Ref(_, t, None) if t.namespace_name_pair() == ("System", "Type") => {
                FieldOrPropType::SystemType
            }
            Type::Ref(TypeTag::ValueType, t, None) => {
                let resolved = t.resolve(cache).ok_or(crate::DecodeError(
                    "Unresolvable CustomAttribute param TypeDefOrRef",
                ))?;
                if !resolved.is_enum() {
    ...
    fn parse<'c: 'db>(
        ...
        Ok(match cur.read_u8()? {
            ...
            bits::ARG_ENUM => {
                let type_string =
                    read_string(cur)?.expect("NamedArg enum type name must not be NULL");
                let type_def = match type_string.resolve(cache) {
                    None => {
                        return Err(
                            "CustomAttribute named param referenced unresolved enum type".into(),
                        )
                    }
                    Some(t) => {
                        if !t.is_enum() {
                            return Err(
                                "CustomAttribute named param referenced non-enum type".into()
                            );
                        } else {
                            t
                        }
    ...
    fn parse_value(self, cur: &mut &'db [u8]) -> Result<Elem<'db>> {
        ...
        Ok(match self {
            ...
            SystemType => Elem::SystemType(
                read_string(cur)?.expect("NULL string in System.Type custom attribute value"),
            ),
            Enum(t) => {
...
impl<'db> ElemKind<'db> {
    fn from_fixed_arg_type<'c: 'db>(typ: &Type<'db>, cache: &Cache<'c>) -> Result<ElemKind<'db>> {
        Ok(match typ {
            Type::Array(array) => ElemKind::Array(FieldOrPropType::from_fixed_arg_type(
                array.elem_type(),
                cache,
            )?),
